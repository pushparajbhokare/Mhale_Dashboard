...
{   
    ...
    public class QDasDbConnection : DbConnection  {
        ...
        string serialNumColAlarm {get;set;}= "K0014";//Confirm the field is K0054/WV0054 and not K0014/WV0014 //
        ...
        */
        public PartFlowCategoryInput getPartTypeCode(string serialNum){ //gets the partsTypeCode/TETEILNR/PartCategory from serialNum 
            string query = 
            $"SELECT TOP(2) WVTEIL FROM [{QDB_NAME}].[dbo].[WERTEVAR] " +
            $"WHERE {serialNumCol} = '{serialNum}' " + 
            $"ORDER BY WVDATZEIT DESC" ;  
            List<object[]> result = this.GetSingleRowFromSQLquery(query,1);
            if(result.Count>0){
                ...
                for(int i=0; i<input.Count;i++){
                    ...
                    result.Add(key, new string[] 
                                    { getOperatorNameFromId((int)input[i][2]),//(string)input[i][2].ToString()!, //WVPRUEFER : operator name
                                      ...
                                      getMachineNameFromId((int)input[i][0]),//(string)input[i][4].ToString()!, //WVMASCHINE: Machine Name
        ...
        public List<object[]> getQueryinAlarmsSearch(int operationId, string serial){
            string query = $"SELECT DISTINCT ALARM_EW FROM [{QDB_NAME}].[dbo].[ALARM_VALUES] "+
                            $"WHERE K1000={operationId} AND {serialNumColAlarm}='{serial}' " + 
                            $"ORDER BY ALARM_DATETIME DESC" ;
            List<object[]> result = this.ValuesFromSQLquery(query,1);
            return result;
        }

        public List<object[]> getQueryinJoinedAlarmsSearch(int operationId, string serial){
            string query = @"SELECT  
                                    CASE 
                                        WHEN A.ALARM IS NULL THEN 0
                                        WHEN A.ALARM >0 AND A.ALARM <16 THEN 1
                                        WHEN A.ALARM >= 16 AND A.ALARM<=32 THEN 0
                                        WHEN A.ALARM > 32 THEN 1
                                        ELSE  0 
                                        END AS ALM_CHECK 
                                    FROM 
                                        (SELECT [ALARM_EW] AS ALARM FROM 
                                        [{QDB_NAME}].[dbo].[WERTEVAR]  AS CHAR_TABLE 
                                        LEFT JOIN 
                                        [{QDB_NAME}].[dbo].[ALARM_VALUES] AS ALARM_CONTAINER
                                        on ALARM_CONTAINER.K2000=CHAR_TABLE.WVMERKMAL AND
                                        ALARM_CONTAINER.K1000=CHAR_TABLE.WVTEIL AND 
                                        ALARM_CONTAINER.K0014=CHAR_TABLE.WV0014
                                        WHERE 
                                        CHAR_TABLE.WV0014=@SN 
                                        AND CHAR_TABLE.WVTEIL=@OPERATION 
                                        )
                                        AS A;";
                query = query.Replace("@OPERATION", operationId.ToString());
                query = query.Replace("@SN", "'"+serial+"'");
                query = query.Replace("{QDB_NAME}", QDB_NAME); //CorrectionPending : why [{QDB_NAME}].[dbo].[WERTEVAR]]
            List<object[]> result = this.ValuesFromSQLquery(query,1);
            return result ;
        }
        public List<object[]> getQueryToValidateMeasurement(int operationId, string serial){
            string query = @"SELECT  
                            --METEIL, MEMERKMAL, MEUGW, MEOGW, WVWERT, 
                            CASE 
                                WHEN A.WVWERT < A.MEUGW THEN 1 
                                WHEN A.WVWERT > A.MEOGW THEN 1
                                ELSE 0 
                                END AS STAT                          
                                FROM 
                                    (SELECT * FROM 
                                    [{QDB_NAME}].[dbo].[MERKMAL]  AS CHAR_TABLE
                                    LEFT JOIN
                                    [{QDB_NAME}].[dbo].[WERTEVAR]  AS VAL_TABLE
                                    on VAL_TABLE.WVMERKMAL=CHAR_TABLE.MEMERKMAL AND
                                    VAL_TABLE.WVTEIL=CHAR_TABLE.METEIL
                                    WHERE 
                                    VAL_TABLE.WV0014=@SN 
                                    AND VAL_TABLE.WVTEIL=@OPERATION 
                                    )
                                    AS A;";
                query = query.Replace("@OPERATION", operationId.ToString());
                query = query.Replace("@SN", "'"+serial+"'");
                query = query.Replace("{QDB_NAME}", QDB_NAME);
            List<object[]> result = this.ValuesFromSQLquery(query,1);
            return result ;
        }
