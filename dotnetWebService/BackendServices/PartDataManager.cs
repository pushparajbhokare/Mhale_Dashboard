...
namespace PartDataManager{
    ...
    public class PartDataHandler:IpartDataHandler{
        ...
        public async Task<string> getOperationStatus(string opKey,string serial, string timeStamp){
            ...
            /* //followcode is discarded and adapted a new method
            ...
            var items = dbHandler.getQueryinJoinedAlarmsSearch(operationId, serial);
            //var items = dbHandler.getQueryToValidateMeasurement(operationId, serial); // backup plan to find if all the reading are of or not 
            for (int i=0;i<items.Count;i++){
                //System.Console.WriteLine(items[i][0].ToString()!);
                if(System.Int64.Parse(items[i][0].ToString()!)!=0){
                    return "not-ok";
                }
            }
